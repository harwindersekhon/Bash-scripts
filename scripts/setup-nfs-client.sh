#!/usr/bin/env bash
#
# setup-nfs-client.sh - Install the NFS client and mount a remote export interactively.
#
# Asks for the NFS server, lists its exports (showmount -e) to pick from,
# the local mount point and mount options, mounts it, and optionally makes the
# mount persistent in /etc/fstab.
#
...
Install the NFS client and mount an export. Anything not given as an option
is asked interactively (or defaulted with --yes).

Options:
  --server HOST           NFS server hostname or IP
  --export PATH           Export path on the server (default: picked from showmount)
  --mount DIR             Local mount point (default: /mnt/<export name>)
  --options OPTS          Mount options (default: defaults)
  --fstab | --no-fstab    Make the mount persistent in /etc/fstab (default: yes)
...
Examples:
  sudo $0
  sudo $0 --yes --server 192.168.1.10 --export /srv/nfs/share --mount /mnt/share
  sudo $0 --yes --server nas --export /data --options vers=4.2,noatime --no-fstab
EOF
}
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --server) NFS_SERVER=${2:?--server needs a value}; shift ;;
            --export) NFS_EXPORT=${2:?--export needs a value}; shift ;;
            --mount) MOUNT_POINT=${2:?--mount needs a value}; shift ;;
            --options) MOUNT_OPTS=${2:?--options needs a value}; shift ;;
            --fstab) ADD_FSTAB=y ;;
            --no-fstab) ADD_FSTAB=n ;;
...
set_os_vars() {
    case "$OS_FAMILY" in
        rhel) PACKAGES=(nfs-utils) ;;
        debian) PACKAGES=(nfs-common) ;;
    esac
}

# choose_export  Offer the server's export list, or fall back to typing a path.
choose_export() {
    [[ -z "$NFS_EXPORT" ]] || return 0
    local list
    local -a exports=() options=()
    if command -v showmount >/dev/null 2>&1; then
        list=$(timeout 15 showmount -e "$NFS_SERVER" 2>/dev/null || true)
        mapfile -t exports < <(awk 'NR > 1 { print $1 }' <<<"$list")
    fi
    if [[ ${#exports[@]} -gt 0 && "$NONINTERACTIVE" != 1 ]]; then
        local e
        log_info "Exports offered by ${NFS_SERVER}:"
        for e in "${exports[@]}"; do options+=("${e}:${e}"); done
        options+=("other:Type a different path")
        ask_choice "Which export should be mounted?" NFS_EXPORT "${options[@]}"
        if [[ "$NFS_EXPORT" == other ]]; then NFS_EXPORT=""; fi
    elif [[ ${#exports[@]} -eq 0 ]]; then
        log_warn "Could not list exports on ${NFS_SERVER} (NFSv4-only server or firewall) - enter the path manually"
    fi
    ask "Export path on ${NFS_SERVER}" "${exports[0]:-}" NFS_EXPORT
}
...
main() {
    ...
    log_step "Mount setup options"
    ask "NFS server (hostname or IP)" "" NFS_SERVER
    choose_export
    [[ "$NFS_EXPORT" == /* ]] || die "Export path must be absolute"
    ask "Local mount point" "/mnt/$(basename "$NFS_EXPORT")" MOUNT_POINT
    MOUNT_POINT=${MOUNT_POINT%/}
    ask "Mount options" "defaults" MOUNT_OPTS
    ask_yn "Mount automatically at boot (add to ${FSTAB})?" y ADD_FSTAB

    local source="${NFS_SERVER}:${NFS_EXPORT}"

    log_step "Mounting ${source} on ${MOUNT_POINT}"
    run mkdir -p "$MOUNT_POINT"
    local current
    current=$(findmnt -rn -o SOURCE --target "$MOUNT_POINT" 2>/dev/null || true)
    if findmnt -rn --mountpoint "$MOUNT_POINT" >/dev/null 2>&1; then
        if [[ "$current" == "$source" ]]; then
            log_info "${source} is already mounted on ${MOUNT_POINT}"
        else
            die "${MOUNT_POINT} is already a mount point for ${current} - unmount it first or pick another path"
        fi
    else
        run mount -t nfs -o "$MOUNT_OPTS" "$source" "$MOUNT_POINT"
    fi

    if is_yes "$ADD_FSTAB"; then
        log_step "Updating ${FSTAB}"
        if awk -v mp="$MOUNT_POINT" '$1 !~ /^#/ && $2 == mp { found = 1 } END { exit !found }' "$FSTAB"; then
            log_warn "${FSTAB} already has an entry for ${MOUNT_POINT} - leaving it unchanged"
        else
            local fstab_opts=$MOUNT_OPTS
            # _netdev: wait for the network at boot; nofail: don't block boot if the server is down.
            [[ ",${fstab_opts}," == *,_netdev,* ]] || fstab_opts+=",_netdev"
            [[ ",${fstab_opts}," == *,nofail,* ]] || fstab_opts+=",nofail"
            backup_file "$FSTAB"
            append_line_once "$FSTAB" "${source}  ${MOUNT_POINT}  nfs  ${fstab_opts}  0 0"
    ...
    printf '  Mounted     : %s -> %s\n' "$source" "$MOUNT_POINT"
    printf '  Options     : %s\n' "$MOUNT_OPTS"
    printf '  Persistent  : %s\n' "$(if is_yes "$ADD_FSTAB"; then echo "yes (${FSTAB})"; else echo no; fi)"
    printf '  Check       : df -h %s\n' "$MOUNT_POINT"
}
